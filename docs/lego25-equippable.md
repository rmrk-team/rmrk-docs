...
> Note: a base is referenced in an NFT as a separate [resource](/lego2-multi-resource) by specifying
> the base ID and cherry picking the list of parts from the base for that NFT instance. The approach
> differs slightly per implementation, so the examples below will mostly use pseudo-code to
...
1. create a base entity (one-time process for given collection)
2. add a new resource to each target NFT, cherry picking the base's parts
...
This NFT in particular has only a single resource - the equippable base itself. The `base` value
refers to the ID of the Base meta-entity, the `id` of the resource is used for indexing and to be
able to target a specific resource when wanting to replace, accept, or remove it, while `pending`
tells us whether the NFT owner has accepted this resource (false is yes).
...
assigned `base` make up this NFT. The parts themselves are defined in the `base` itself, like so:
...
collections that are allowed as equippables. The issuer of the base can modify this value at any
...
The `z` value in both indicates on which layer the visual appears on the SVG base, height-wise. The
...
Thus, a resource which is a base will cherry pick part from that base to compose a final NFT, until
we get our colorful chunkies.

Our flagship project, Kanaria, uses this same approach. If you wish, you can explore its full base
[here](https://gist.github.com/Yuripetusko/dccd18ef3efe6058d73c29dbe0ef5b6f).
...
by using equippables and multi resources.

If we take for example a Chunky that is level 1, this NFT can have a resource that points to the
Chunky base, but from it picks only the parts that have the `equippable` value set to level-1
...
a fire world, then the next resource could have a fire [theme](#theming). If they are done in a
water world, a water theme could apply. The NFT owner would choose which "quest successes" to equip
into the "Level NFT" which is inside the avatar, and use them to create full "experience sets" this
way.

Once this Chunky has amassed enough experience to go to level 2, a new resource can be added to it,
targeting the previous one by ID to replace it when accepted by the NFT owner. This resource will
point to the same base, but will now pick slot parts that are configured to accept level 1 and level
2 collections of items, thereby making it possible to now equip higher level gear.
...
- [EVM](https://github.com/rmrk-team/evm/tree/dev/contracts/RMRK/equippable)
